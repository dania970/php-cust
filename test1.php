...
class User
{
    ...
    // If you want to use a database, uncomment and complete this method
    /*
    public function validateWithDatabase($dbConnection) {
        $stmt = $dbConnection->prepare("SELECT * FROM users WHERE username = :username AND password = :password");
        $stmt->execute(['username' => $this->username, 'password' => md5($this->password)]);
